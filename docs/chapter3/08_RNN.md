...
> 为了解决这些问题，许多 RNN 的变体被提出。例如，为了让模型能同时利用过去和未来的上下文信息的 **双向循环神经网络（Bidirectional RNN, BiRNN）** ；通过将多个 RNN 层堆叠起来， **深度循环神经网络（Deep RNN）** 能学习到更复杂的特征表示。而其中两种最为著名的变体——**长短期记忆网络（Long Short-Term Memory, LSTM）** 和 **门控循环单元（Gated Recurrent Unit, GRU）**——则旨在解决核心的长距离依赖问题，后续章节将对此进行详细探讨。
