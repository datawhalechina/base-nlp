...
可以将这个过程看作是对全连接网络的改进：
1.  **当前输入的信息**：和之前一样，用一个全连接层处理当前输入 $x_t$，得到 $U \cdot x_t$。（$U$ 对应 `weight_ih`）
2.  **过去记忆的信息**：引入另一个全连接层，处理来自上一步的隐藏状态 $h_{t-1}$，得到 $W \cdot h_{t-1}$。（$W$ 对应 `weight_hh`）
...
下图展示了 RNN 单元（RNN Cell）在单个时间步内的计算流程。需要注意，为了可视化，此图将隐藏状态拆分成了两个部分（`Hidden State` 和 `Hidden Layer`），但在计算上它们是紧密关联的。
...
4.  **状态更新**: 新计算出的隐藏状态 $h_t$ 会被传递给下一个时间步，成为新的 $h_{t-1}$（图中从 "Hidden Layer" 指向 "Hidden State" 的紫色虚线）。
5.  **输出 ($o_t$)**: 当前的隐藏状态 $h_t$ 也可以被用来计算当前步的最终输出（如图中从 "Hidden Layer" 指向 "Output" 的橙色虚线），这通常需要再经过一个独立的输出层（权重为 $V$）。
...
-   **静态输入 (Static Input)**: RNN的输入 $x_t$ (如“周杰伦”的词向量) 是静态的。无论“周杰伦”出现在哪个句子里，这个从Word2Vec等模型中查出来的初始向量都是固定不变的。

-   **动态表示 (Dynamic Representation)**: RNN的输出——隐藏状态 $h_t$ ——是动态的。
...
> RNN 通过其循环结构，将静态的词向量融入了不断演变的上下文信息，从而为序列中的每一个词元都生成了一个独一无二的、动态的、上下文相关的向量表示。这正是 RNN 的核心价值所在。

## 四、从零实现一个 RNN

为了将理论付诸实践并加深理解，接下来从最底层的数学公式出发，用 NumPy 手写一个 RNN。
...
def prepare_inputs():
    ...
    return tokens, x_np
```
这段代码首先定义了一个简单的词表，然后为句子中的每个词生成一个随机的词向量，最终组合成一个形状为 `(1, 4, 128)` 的张量，作为 RNN 模型的输入。

### 4.3 基于 NumPy 实现 RNN

首先用 NumPy 严格按照上述公式实现计算过程。依照前文 RNN 结构，将隐藏节点设置为3个 (`H=3`)，以便于理解（实际使用不会这么少）。
...
```
这段代码直观地展示了RNN如何通过一个`for`循环，一步步地处理序列信息，并将历史信息（`h_prev`）和当前输入（`x_t`）结合起来。
...
为了解决这些问题，许多 RNN 的变体被提出，其中最著名和最成功的就是 **长短期记忆网络（Long Short-Term Memory, LSTM）** 和 **门控循环单元（Gated Recurrent Unit, GRU）**，后续章节将对此进行详细探讨。 