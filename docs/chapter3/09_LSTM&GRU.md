...
在深入公式之前，我们可以将上图中的核心计算模块与公式对应起来，以便更清晰地理解信息流：
- 图中每一个 `σ` (Sigmoid) 符号都对应一个**门**的计算，即遗忘门、输入门和输出门。
- 图中的 `tanh` 符号对应**候选记忆**的生成和对细胞状态的“压制”处理。
- 图中的 `x` (圆圈) 符号代表**按元素乘法 (Hadamard Product, $\odot$)**，这是门控机制发挥作用的关键。
...
> 细胞状态的更新方式 $c_t = f_t \odot c_{t-1} + \dots$ 与深度学习中著名的残差网络结构有异曲同工之处。它建立了一条从 $c_{t-1}$ 到 $c_t$ 的直接连接（类似于残差连接中的“恒等映射”），其权重由可学习的遗忘门 $f_t$ 控制。这种结构保证了即使在深度网络（或长序列）中，也存在一条通畅的梯度传播路径，从而有效缓解梯度消失问题。

因此，我们不说 LSTM *解决* 了梯度消失问题，而是极大地 *缓解* 了它。因为即使 $f_t$ 的值很接近1（例如0.99），在足够长的序列上连乘后，梯度依然会衰减。但相比简单 RNN 几十步就会出现问题的窘境，LSTM 已经实现了巨大的飞跃。
...
同样，我们实现一个不含偏置项的简化版 LSTM，其计算公式如下：
...
在 LSTM 提出后不久，Cho 等人在 2014 年提出了门控循环单元 (GRU)，作为 LSTM 的一个流行变体。GRU 的设计目标是**在保持 LSTM 核心优势（缓解长距离依赖）的同时，简化其内部结构，使其参数更少，计算效率更高**。在许多任务上，GRU 都能取得与 LSTM 相媲美的性能。

### 7.1 GRU 的核心思想

GRU 对 LSTM 做了两大核心改动：

1.  **合并细胞状态与隐藏状态**：GRU 不再区分细胞状态 $c_t$ 和隐藏状态 $h_t$。它只有一个状态向量 $h_t$ 在时间步之间传递。这个 $h_t$ 既包含了长期记忆，也作为当前时间步的输出，类似于简单 RNN 的结构。
...
4.  **最终隐藏状态 ($h_t$)**
    ...
    -   $(1-z_t) \odot h_{t-1}$：这部分表示对旧状态 $h_{t-1}$ 中需要**遗忘**的信息。
    -   $z_t \odot \tilde{h}_t$：这部分表示从候选记忆 $\tilde{h}_t$ 中需要**选择**的新信息。
    
    这个更新机制非常巧妙：更新门 $z_t$ 的值在 0 到 1 之间，可以看作是一个“开关”。当 $z_t$ 接近 1 时，模型倾向于保留更多的新信息（候选记忆）；当 $z_t$ 接近 0 时，模型则倾向于保留更多的旧信息 $h_{t-1}$。
