...
2.  **微调 (Fine-tuning)**
    -   当面临一个具体的下游任务时（如文本分类、命名实体识别），我们不再从头开始训练模型。
    -   相反，把预训练好的 BERT 模型作为任务模型的 **基础结构**。加载它已经学习到的所有参数作为 **初始值**。
    -   然后，根据具体任务，在 BERT 模型之上增加一个小的、任务相关的输出层（例如，一个用于分类的全连接层）。
    -   最后，在自己的任务数据集上对整个模型（或仅仅是顶部的输出层）进行训练。由于模型已经具备了强大的语言理解能力，这个微调过程通常非常快速，并且只需要相对较少的数据就能达到很好的效果。
...
### 5.2 Hugging Face Transformers 生态
