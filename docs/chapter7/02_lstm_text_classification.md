...
显然结果并不符合我们的预期，理论上更能捕捉序列信息的 LSTM 模型，在本次新闻分类任务上的表现反而**劣于**简单的全连接模型。这个发现说明**模型的复杂性与任务的实际需求应该匹配**。值得一提的是，本次对比实验并未严格控制固定随机数种子，所以每次运行的结果会存在细微的差别。然而，一个稳定的现象是，引入序列建模的 LSTM 并未带来性能提升，其结果反而总是比简单的全连接模型低 **~2%** 左右，这足以让我们自信地得出以下结论。
