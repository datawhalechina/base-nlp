...
这个序列本身并不直观。为了进行实体级评估，或者将预测结果呈现给用户，我们必须实现一个 **解码 (Decode)** 函数，将这个数字序列转换成一个包含具体实体信息的列表，例如：`[{"text": "高血压", "type": "dis", "start": 3, "end": 6}]`。

这个解码过程的核心，就是根据 `BMES` 标注体系的规则，从标签序列中解析出实体的边界和类型。

### 2.1 解码逻辑详解

解码函数需要遍历标签序列，并像一个“状态机”一样，根据当前遇到的标签（`B`, `M`, `E`, `S`, `O`）来维护一个 `current_entity` 对象。
...
最后，将所有推理相关的逻辑（加载模型、文本预处理、模型预测、结果解码）封装到一个 `NerPredictor` 类中，使其成为一个开箱即用的独立组件。
...
1.  **加载配置**: 从模型目录加载 `config.json`，获取模型超参数和相关文件路径。
    > **[开发插曲] 确保训练与推理的配置同步**
    >
    > 在编写 `NerPredictor` 时，遇到了一个典型的问题：推理脚本需要知道训练时使用的模型配置（如 `hidden_size` 等）才能正确地重建模型，但之前的训练脚本 `05_train.py` 并没有将这些配置信息保存下来。
    >
    ...
    > ```
    >
    > 完成这个“补丁”后，我们就可以放心地实现 `NerPredictor` 的核心流程了。

2.  **加载词汇表和标签映射**: 根据配置文件中的路径，加载 `vocabulary.json` 和 `tags.json`，并构建 `id2tag` 映射。
3.  **加载分词器**: 初始化 `CharTokenizer`。
4.  **初始化模型并加载权重**:
    -   根据配置实例化 `BiGRUNerNetWork` 模型。
    -   从模型目录加载 `best_model.pth` 模型权重。**注意**：这里需要使用 `map_location=self.device` 来确保模型可以被加载到指定的设备上（无论是 CPU 还是 GPU），这对于在没有 GPU 的环境中进行推理至关重要。
    -   调用 `model.to(self.device)` 将模型移至指定设备。
    -   调用 `model.eval()` 将模型切换到**评估模式**。这是一个非常重要的步骤，它会关闭 Dropout 和 BatchNorm 等只在训练时使用的层，确保预测结果的确定性。
...
我们前面实现的 `NerPredictor` 非常适合处理单句、实时的预测请求。然而，当我们在训练过程中需要在验证集上进行批量评估时，情况会变得更复杂。`Trainer` 每次会向模型输入一个批次（batch）的数据，这引入了两个在单句推理中不存在的新问题：**填充（Padding）** 和 **样本来源追踪（Sample ID）**。

### 4.1 处理填充（Padding）

一个批次中的句子长度不同，为了组成一个规整的张量，短句子会被填充到和最长的句子一样长。这些填充位对应的标签（在我们的项目中通常是 `-100`）在计算损失时会被忽略，但在解码实体时也必须被显式地处理。

如果我们不对填充标签做特殊处理，解码函数可能会错误地将它们计入序列长度。一个稳健的批量解码函数，在遍历标签序列时，一旦遇到填充 ID，就应该立即停止对当前样本的处理。

### 4.2 追踪样本来源（Sample ID）

当解码函数处理一个批次的标签矩阵（例如 `[32, 128]`，32个样本，长度128）时，它会为这个批次内的所有样本抽取出实体。这时，我们必须知道每个提取出的实体到底属于批次中的哪一个句子。因此，解码函数的输出需要包含样本的索引，通常是一个四元组 `(sample_id, start, end, type)`。

更进一步，当我们在整个验证集上进行评估时（这会包含很多个批次），第一个批次的 `sample_id` 是 0-31，第二个批次也从 0-31 开始，这会导致 ID 冲突。正确的做法是引入一个 `offset`，在处理每个新批次时，将其 `sample_id` 加上这个偏移量，确保每个样本在整个验证集上都有一个独一无二的全局 ID。

## 五、进阶策略：自定义损失函数对抗数据不均衡

第一节中我们提到，数据不均衡是 NER 任务中的一个核心挑战。仅仅依赖实体级评估指标是在“下游”解决问题，我们还可以在“上游”——即损失函数的设计上，主动地引导模型去关注那些少数但重要的实体样本。

标准的交叉熵损失函数对所有 Token 一视同仁，当 `'O'` 标签占据 95% 的时候，损失值自然会被这些“多数派”主导。下面介绍两种在课程中提到的自定义损失函数策略，来缓解这个问题。
